...
def get_time_sampler(config):
  ...
  def sample_uniformly(bs, state):
    u = (state.u0 + math.sqrt(2)*jnp.arange(bs*jax.device_count())) % 1 # arange с иррациональным шагом и взятие дробной части
    new_state = state.replace(u0=u[-1:]) # 
    t = (t_1-t_0)*u[jax.process_index()*bs:(jax.process_index()+1)*bs] + t_0 # Масштабирование и сдвиг
    return t, new_state

  def uniform_invdensity(t):
    return jnp.ones_like(t)*(t_1 - t_0) # обратная плотность распределения
