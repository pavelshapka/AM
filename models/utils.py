...
def init_model(rng, config): # (jax_random_key, config)
  """ Initialize a `flax.linen.Module` model. """
  model_name = config.model.name # anet
  ...
  fake_x = jnp.zeros(x_shape) # fake_t и fake_x — фиктивные входные данные, используемые для определения размерности выходов.
  ...
  initial_params = variables['params'] # параметры модели, которые обучаются
  init_model_state = variables.copy() # состояние модели, не обучается, используется для forward pass
  del init_model_state['params']
