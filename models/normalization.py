...
# нормализация на дисперсию x / sigma
class VarianceNorm2d(nn.Module):
  ...
  def scale_init(key, shape, dtype=jnp.float32):
    normal_init = init.normal(0.02) # mu = 0, sigma = 0.02; Возвращает инициализатор
    return normal_init(key, shape, dtype=dtype) + 1. # mu = 1, sigma = 0.02

  @nn.compact
  def __call__(self, x):
    variance = jnp.var(x, axis=(1, 2), keepdims=True) # дисперсия по осям 1 и 2
    h = x / jnp.sqrt(variance + 1e-5)

    h = h * self.param('scale', VarianceNorm2d.scale_init, (1, 1, 1, x.shape[-1])) # Инициализирует параметр 'scale'; каждый канал масштабируется
    if self.bias:
      h = h + self.param('bias', init.zeros, (1, 1, 1, x.shape[-1]))

    return h

# Стандартная нормализация (x - mu) / sigma
class InstanceNorm2d(nn.Module):
  ...
  def __call__(self, x):
    ...
    h = h * self.param('scale', init.ones, (1, 1, 1, x.shape[-1])) # нормализация каждого канала
    if self.bias:
      h = h + self.param('bias', init.zeros, (1, 1, 1, x.shape[-1]))

    return h

# Как стандартная нормализация, но 1-й и 4-й размерности нормализуются еще на mean_plus (условная нормализация)
class InstanceNorm2dPlus(nn.Module):
  ...
  def __call__(self, x):
    ...
    means_plus = (means - m) / jnp.sqrt(v + 1e-5) # нормализация среднего (1, 2) по каналам (3)

    h = (x - means[:, None, None, :]) / jnp.sqrt(jnp.var(x, axis=(1, 2), keepdims=True) + 1e-5) # обычная нормализация

    h = h + means_plus[:, None, None, :] * self.param('alpha', InstanceNorm2dPlus.scale_init, (1, 1, 1, x.shape[-1])) # смещение; alpha
    h = h * self.param('gamma', InstanceNorm2dPlus.scale_init, (1, 1, 1, x.shape[-1])) # gamma
    if self.bias:
...
# Нормализация данных:
# 1. Данные x нормализуются по их среднему значению и дисперсии.
# 2. Эмбеддинг меток классов:
# 3. Метки классов y преобразуются в эмбеддинги с помощью nn.Embed.

# Условная нормализация:
# 1. Эмбеддинги разделяются на параметры gamma, alpha, beta.
# 2. Нормализованные данные масштабируются и смещаются с использованием параметров gamma, alpha, beta.
class ConditionalInstanceNorm2dPlus(nn.Module):
  ...
  def __call__(self, x, y):
    ...
    if self.bias: # применение нормализации, как в предыдущей функции
